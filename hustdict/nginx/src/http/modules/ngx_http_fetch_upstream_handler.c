...
{
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "fuck, why am i here!@#$%^&*()_+");
    return;
}
...
void ngx_http_fetch_upstream_finalize_request(ngx_http_request_t *r, ngx_int_t rc)
{
    ngx_http_fetch_ctx_t *ctx;
    ctx = ngx_http_fetch_get_module_ctx(r);
    ctx->reuse(r->connection);
    if (ctx->post_request_handler)
    {
        ctx->post_request_handler(r, ctx->data, rc);
    }
    return;
}
