...
static void __init_upstream_conf(const ngx_http_fetch_upstream_conf_t * src, ngx_http_upstream_conf_t * des)
{
    des->connect_timeout = (src && 0 != src->connect_timeout) ? src->connect_timeout : 6000;
    des->send_timeout = (src && 0 != src->send_timeout) ? src->send_timeout : 6000;
    des->read_timeout = (src && 0 != src->read_timeout) ? src->read_timeout : 6000;
    des->timeout = (src && 0 != src->timeout) ? src->timeout : 60000;

    des->buffering = 1;
    des->buffer_size = (src && 0 != src->buffer_size) ? src->buffer_size : ngx_pagesize;
    des->bufs.num = (src && 0 != src->bufs.num) ? src->bufs.num : 8;
    des->bufs.size = (src && 0 != src->bufs.size) ? src->bufs.size : ngx_pagesize;
    des->busy_buffers_size = (src && 0 != src->busy_buffers_size) ? src->busy_buffers_size : 2 * des->buffer_size;
}
...
ngx_int_t ngx_http_fetch(const ngx_http_fetch_args_t * args, const ngx_http_auth_basic_key_t * auth)
{
    ...
    ngx_http_request_t * r = ngx_http_fetch_create_request(
        g_conf->request_pool_size, g_conf->variables, g_conf->cscf->ctx, g_conf->cycle);
    if (!r)
    {
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
    }
    ...
    ngx_http_fetch_upstream_data_t upstream_data = { args, auth, &g_conf->headers_in_hash, &g_conf->schema, &g_conf->umcf, r };
